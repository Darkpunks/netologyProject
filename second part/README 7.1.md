...
1) Присутствуют образы Docker и Packer, следовательно тип инфраструктуры: не изменяемый. 
2) Инструменты: образы Docker (контейнеры)
3) Платформа предоставления серверных мощностей (серверов), использовал Kuber, как решение оркестрации контейнеров, с планами на масштабирование.
4) Ansible - инструмент подготовки ENV внутри уже подготовленных. 
5) Terraform - применял бы в части Cloud решений. 
6) Часть сервиса, что располагается в Public Cloud - для инициализации ресурсов.
7) Private Cloud - управление конфигурацией. 
